...
Namespace geshotelk
    Partial Public Class GesHotelClase
        ...
        Public Function enviarNotificacionesPendientes() As Boolean
            ...
            Try
                ...
                For x = 0 To noti.Rows.Count - 1
                    ...
                    cmd.Parameters.Clear()
                    Dim dispositivo_idParam As New SqlParameter("@dispositivo_id", row.Item("dispositivo_id"))
        ...
        Public Function enviarNotificacion(disp() As Integer, mensaje As String) As Boolean
            ...
            Try
                cmd.Parameters.Clear()
                Dim notificacion_idParam As New SqlParameter("@notificacion_id", -1)
        ...
        Private Function crearNuevoDisp(cmd As SqlCommand, disp As Dispositivo, dispositivo_id As Integer)
            cmd.Parameters.Clear()
            Dim dispositivo_idParam As New SqlParameter("@dispositivo_id", dispositivo_id)
            cmd.Parameters.Add(dispositivo_idParam)
            ...
            writer.Update(ds.Tables(0))
        End Function
        Public Function registrarDispositivo(cmd As SqlCommand, disp As Dispositivo)
            Dim retVal As Boolean = False
            Dim errorCode As Integer = 0
            Try
        ...
        Private Function ObtieneDispositivo(cmd As SqlCommand, disp As Dispositivo) As Integer
       ...
       & "and (destino=?)"
        ...
        Private Function ObtieneDispositivos(cmd As SqlCommand, filtrodisp As Dispositivo) As Integer()
            Dim errorCode As Integer = 0
            ...
            Try
                ...
                If filtrodisp.usuario_id = 0 And filtrodisp.cliente_id = 0 And filtrodisp.hotel_id = 0 And filtrodisp.reserva_id = 0 Then
                    Dim tipoParam As New SqlParameter("@tipo", filtrodisp.tipo)
                    cmd.Parameters.Add(tipoParam)
                    Dim destinoParam As New SqlParameter("@destino", filtrodisp.destino)
                    cmd.Parameters.Add(destinoParam)
                    reader = getDataTable(cmd, sqlDispositivosFiltroUnknown)
                Else
                    Dim usuario_idParam As New SqlParameter("@usuario_id", filtrodisp.usuario_id)
                    Dim usuario_id1Param As New SqlParameter("@usuario_id1", filtrodisp.usuario_id)
                    cmd.Parameters.Add(usuario_idParam)
                    cmd.Parameters.Add(usuario_id1Param)
                    Dim cliente_idParam As New SqlParameter("@cliente_id", filtrodisp.cliente_id)
                    Dim cliente_id1Param As New SqlParameter("@cliente_id1", filtrodisp.cliente_id)
                    cmd.Parameters.Add(cliente_idParam)
                    cmd.Parameters.Add(cliente_id1Param)
                    Dim hotel_idParam As New SqlParameter("@hotel_id", filtrodisp.hotel_id)
                    Dim hotel_id1Param As New SqlParameter("@hotel_id1", filtrodisp.hotel_id)
                    cmd.Parameters.Add(hotel_idParam)
                    cmd.Parameters.Add(hotel_id1Param)
                    Dim reserva_idParam As New SqlParameter("@reserva_id", filtrodisp.reserva_id)
                    Dim reserva_id1Param As New SqlParameter("@reserva_id1", filtrodisp.reserva_id)
