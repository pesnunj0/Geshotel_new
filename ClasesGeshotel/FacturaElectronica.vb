...
Namespace geshotelk
    Partial Public Class GesHotelClase
        ...
        Public Function generarEfacturaBavel(ByVal factura_id As Integer, Optional ByRef fb As facbavel = Nothing, Optional ByVal soloProbar As Boolean = False) As String
            ...
            Return retVal
        End Function
        Private Function generarEfacturaBavel(ByVal cmd As SqlCommand, ByVal factura_id As Integer, Optional ByRef fb As facbavel = Nothing, Optional ByVal soloProbar As Boolean = False) As String
            Dim retVal As String = Nothing
            Dim errorCode As Integer = 0
            ...
            Try
                ...
                Dim factura_idParam As New SqlParameter("@factura_id", factura_id)
                ...
                If Not IsNothing(cabfactura) Then
                    ...
                    If cabfactura("estado_factura_id") <> 0 Then
                        ...
                        Dim cliente_idParam As New SqlParameter("@cliente_id", cliente_id)
                        ...
                        If Not IsDBNull(tipo_bavel) Then
                            If tipo_bavel = 99 Then ' Aqui si la factura es de pension, za,za,zasca!
                                ...
                                Dim tipo_servicio_idParam As New SqlParameter("@tipo_servicio_id", 1)  ' A piñon 1 Habitacion
                                cmd.Parameters.Clear()
                                cmd.Parameters.Add(factura_idParam)
                        ...
                        If datoshotelcompletos Then
                            ...
                            Try
                                ...
                                If Not cabfactura.IsNull("id_factura_rectificada") Then
                                    ...
                                    cmd.Parameters.Clear()
                                    Dim facturarec_idParam As New SqlParameter("@factura_id", cabfactura("id_factura_rectificada"))
                                    ...
                                    Try
                                        ...
                                        Else
                                            ...
                                            If Not cabcliente.IsNull("agencia_id") Then
                                                cmd.Parameters.Clear()
                                                Dim agencia_idParam As New SqlParameter("@agencia_id", cabcliente("agencia_id"))
        ...
        Private Function ObtieneProvincia(ByVal cmd As SqlCommand, ByVal provincia_id As Integer) As String
            cmd.Parameters.Clear()
            Dim provincia_idParam As New SqlParameter("@provincia_id", provincia_id)
        ...
        Private Function ObtienePais(ByVal cmd As SqlCommand, ByVal nacion_id As Integer) As String
            cmd.Parameters.Clear()
            Dim nacion_idParam As New SqlParameter("@nacion_id", nacion_id)
        ...
        End Class
        Function generaFicheroBavel(ByVal cmd As SqlCommand, ByVal factura_id As Integer) As Boolean
        ...
        Function generarFicherosBavelCliente(ByVal cliente_id As Integer) As Boolean
            ...
            Try
                ...
                Dim cliente_id1Param As New SqlParameter("@cliente_id1", cliente_id)
            ...
            Return True
        End Function
        Function generarFicherosBavelHotel(ByVal cmd As SqlCommand, ByVal hotel_id As Integer) As Boolean
            'en cada cierre de hotel generar las facturas bavel de ese hotel
            Dim errorCode As Integer = 0
            Dim reader As DataTableReader = Nothing
            Try
        ...
        End Function
        Function ColocaEstadoBavel(ByVal cmd As SqlCommand, ByVal factura_id As Integer, Optional ByVal enviar As Boolean = True) As Boolean
            Dim retVal As Boolean = False
            Dim errorCode As Integer = 0
            Try
                cmd.Parameters.Clear()
                ...
                If EsClienteBavel(cmd, cabfactura("cliente_id") And (cabfactura("serie_id") = 1 Or (cabfactura("serie_id") = 9 And cabfactura("importe_total") > 0)), cabfactura("hotel_id")) And cabfactura("importe_total") <> 0 Then
                    ...
                    Dim sql_tipo_factura = "select tipo_bavel from clientes_hotel where cliente_id = ? and hotel_id = ?"
                    cmd.Parameters.Clear()
                    Dim cliente_idParam As New SqlParameter("@cliente_id", cabfactura("cliente_id"))
                    Dim hotel_idParam As New SqlParameter("@hotel_id", cabfactura("hotel_id"))
                    cmd.Parameters.Add(cliente_idParam)
                    cmd.Parameters.Add(hotel_idParam)
                    Dim tipo_bavel = ExecuteScalar(cmd, sql_tipo_factura)
                    If Not IsDBNull(tipo_bavel) Then
                        ...
                        Else ' Solo se quieren enviar las de tipo habitación o pensión
                            ...
                            & "Inner Join servicios ON lineas_factura.servicio_id = servicios.servicio_id " _
                            & "WHERE lineas_factura.factura_id = ? AND servicios.tipo_servicio_id = ? "
                            Dim tipo_servicio_idParam As New SqlParameter("@tipo_servicio_id", tipo_bavel)
                            cmd.Parameters.Clear()
                            cmd.Parameters.Add(factura_idParam)
                            cmd.Parameters.Add(tipo_servicio_idParam)
                            Dim importe = ExecuteScalar(cmd, sql_importe_tipo_servicio)
        ...
        Function EsClienteBavel(ByVal cliente_id As Integer, ByVal hotel_id As Integer) As Boolean
            ...
            Return retVal
        End Function
        Function EsClienteBavel(ByVal cmd As SqlCommand, ByVal cliente_id As Integer, ByVal hotel_id As Integer) As Boolean
            Dim retVal As Boolean = False
            Dim errorCode As Integer = 0
            Try
